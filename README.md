...
Finally, it is worht noting that MM2SF also accounts for an old formatting (deprecated) for the computation of the ACSF features. Such a formating relies on unique integer identifiers to refer to the atomic pairs or atomic trios. If desired, the ACSF features can be computed with this deprectaed fromat as:
...
Notice that the input.rad and input.ang files used in this case have to be generated by setting the new_format=False flag during the exploration of the radial and angular spaces. Both the current and deprecated formats will produce exactly the same features so the use of the latter is generally discoraged for the general user.
